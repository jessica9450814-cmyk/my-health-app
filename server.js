...
app.get('/dashboard', (req, res) => {
    ...
    res.sendFile(p, (err) => {
        if (err) res.status(404).send("找不到 dashboard.html 檔案");
    });
});
...
app.post('/api/chat', async (req, res) => {
    ...
    try {
        // 使用 gemini-1.5-flash，這是目前最推薦且穩定的模型
        ...
        res.status(500).json({ error: "AI 服務無法回應，請檢查 API 金鑰權限。" });
    }
});
